...
TOP_1M_PATH = PROJECT_ROOT / "data" / "raw" / "top-1m.csv"
_TOP_1M_RANKS = None
...
def _load_top_1m_rank_map() -> dict:
    """Load rank map from top-1m CSV as {domain: rank} (lazy singleton)."""
    global _TOP_1M_RANKS
    if _TOP_1M_RANKS is not None:
        return _TOP_1M_RANKS

    ranks = {}
    try:
        if TOP_1M_PATH.exists():
            with TOP_1M_PATH.open("r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    row = line.strip()
                    if not row or "," not in row:
                        continue
                    rank_s, domain = row.split(",", 1)
...
def get_top_rank(url_or_host: str) -> int | None:
    """Return Top-1M rank for registrable domain, or None if unavailable."""
    ...
    if not reg:
        return None
    return _load_top_1m_rank_map().get(reg)


def is_top_ranked_domain(url_or_host: str, max_rank: int = 20000) -> bool:
    """Check whether domain appears within top N of loaded Top-1M dataset."""
    rank = get_top_rank(url_or_host)
    return rank is not None and int(rank) <= int(max_rank)
