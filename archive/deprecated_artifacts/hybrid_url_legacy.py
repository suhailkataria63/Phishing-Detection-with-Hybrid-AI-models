...
class HybridURLModel:
    ...
    def predict(self, url: str, enable_explain: bool = True):
        ...
        # ---- v1 prediction ----
        out_v1 = self.v1.predict(normalized, enable_explain=enable_explain)
        v1_score = float(out_v1.get("probability", 0.0))
        v1_label = out_v1.get("label", "legitimate")
        reasons = list(out_v1.get("reasons", [])) if enable_explain else []
        ...
        has_hard_cue = any(r.get("feature") in HARD_CUES for r in reasons) if enable_explain else False
        has_soft_only = (not has_hard_cue) and any(r.get("feature") in SOFT_CUES for r in reasons) if enable_explain else False
